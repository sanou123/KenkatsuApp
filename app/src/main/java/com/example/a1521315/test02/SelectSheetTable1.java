...
public class SelectSheetTable1 extends AppCompatActivity implements View.OnFocusChangeListener, SearchView.OnQueryTextListener {
    ...
    public boolean onQueryTextSubmit(String query) {
        ...
        // ヘッダー：産地
        TextView headerHeart_rate = setTextItem("最大\n心拍数", GCH);            // TextViewのカスタマイズ処理
        TableRow.LayoutParams paramsHeart_rate = setParams(0.4f);       // LayoutParamsのカスタマイズ処理
        // ヘッダー：個数
        TextView headerCalorie_consumption = setTextItem("消費\nカロリー", GCH);
        ...
        rowHeader.addView(headerCalorie_consumption, paramsCalorie_consumption);          // ヘッダー：個数
        rowHeader.addView(headerWeight_fluctuates, paramsWeight_fluctuates);            // ヘッダー：単価
        rowHeader.addView(headerTotal_time, paramsTotal_time);          // ヘッダー：個数
        rowHeader.addView(headerTotal_distance, paramsTotal_distance);            // ヘッダー：単価
        rowHeader.setBackgroundResource(R.drawable.row_deco1);  // 背景

        // TableLayoutにrowHeaderを追加
        mTableLayout04List.addView(rowHeader);

        String column = "heart_rate";          //検索対象のカラム名
        ...
        if (c.moveToFirst()) {
            do {
                ...
                // 産地
                TextView textHeart_rate = setTextItem(c.getString(2), GCH);     // TextViewのカスタマイズ処理
                // 個数
                TextView textCalorie_consumption = setTextItem(c.getString(3), GE);      // TextViewのカスタマイズ処理
                // 単価
                TextView textWeight_fluctuates = setTextItem(c.getString(4), GE);      // TextViewのカスタマイズ処理
                // 個数
                TextView textTotal_time = setTextItem(c.getString(5), GE);      // TextViewのカスタマイズ処理
                // 単価
                TextView textTotal_distance = setTextItem(c.getString(6), GE);      // TextViewのカスタマイズ処理

                // rowHeaderに各項目(DBから取得した産地,個数,単価)を追加
                row.addView(textHeart_rate, paramsHeart_rate);      // 産地
                row.addView(textCalorie_consumption, paramsCalorie_consumption);      // 個数
                row.addView(textWeight_fluctuates, paramsWeight_fluctuates);        // 単価
                row.addView(textTotal_time, paramsTotal_time);      // 個数
                row.addView(textTotal_distance, paramsTotal_distance);        // 単価
