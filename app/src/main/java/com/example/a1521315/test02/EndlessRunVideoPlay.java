...
public class EndlessRunVideoPlay extends Activity implements SurfaceHolder.Callback, Runnable, MediaPlayer.OnCompletionListener,View.OnClickListener {
    ...
    /*シークバーに関する奴(シークバーはないがここでは平均速度出すのに必要)*/
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                Log.d("", "ACTION_DOWN");
                if (findViewById(R.id.ConnectCheak).getVisibility() == View.INVISIBLE) {//トレーニングが始まっているときのみ画面タップでポーズする
                    pauseDialog();
                }
                break;
        }
